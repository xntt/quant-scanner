...
class EastMoneyFetcher:
    ...
    def _get_board_code(self, board_name: str, board_type: str) -> str:
        ...
        return match.iloc[0]["board_code"] if not match.empty else ""
    ...
    def _fetch_board_list(self, b_type: str) -> pd.DataFrame:
        ...
        try:
            ...
            st.warning(f"获取{b_type}板块列表失败: {e}")
            return pd.DataFrame()
    ...
    def get_board_history(_self, board_name: str, board_type: str = "concept", days: int = 10) -> pd.DataFrame:
        ...
        params = {
            ...
            "fqt": "1",          # 1代表前复权
            "lmt": str(days),    # 获取的天数
            "end": "20500000",   # 获取到最新日期
            "iscca": "1",
            "fields1": "f1,f2,f3,f4,f5,f6",
            "fields2": "f51,f52,f53,f54,f55,f56,f57,f58,f59,f60,f61"
        }
        try:
            resp = self.session.get(_self.KLINE_URL, params=params, timeout=10)
            data = resp.json()
            if not data or not data.get("data") or not data["data"].get("klines"):
                return pd.DataFrame()

            klines = data["data"]["klines"]
            # 解析K线字符串: 日期,开盘,收盘,最高,最低,成交量,成交额,振幅,涨跌幅,涨跌额,换手率
    ...
    def get_sector_fund_flow(_self, sector_type: str = "concept") -> pd.DataFrame:
        """直接请求东方财富板块资金流API (核心数据)"""
        fs_map = {"concept": "m:90+t:3+f:!50", "industry": "m:90+t:2+f:!50"}
        params = {
            "pn": 1, "pz": 200, "po": 1, "np": 1, 
            "fltt": 2, "invt": 2, "fid": "f62",  # f62 代表主力净流入
            "fs": fs_map.get(sector_type, fs_map["concept"]),
            "fields": "f12,f14,f2,f3,f62,f184,f66,f69,f72,f75,f78,f81,f84,f87,f204,f205,f124",
        }
        try:
            resp = self.session.get(_self.LIST_URL, params=params, timeout=10)
            data = resp.json()
            if not data or not data.get("data") or not data["data"].get("diff"):
                return pd.DataFrame()
