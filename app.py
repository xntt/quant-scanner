...
if 'scan_pool' not in st.session_state:
    st.session_state['scan_pool'] = [] # 全市场股票池
if 'scanned_count' not in st.session_state:
    st.session_state['scanned_count'] = 0 # 已经扫了多少只（断点位置）
if 'is_scanning' not in st.session_state:
    st.session_state['is_scanning'] = False # 扫描引擎开关
...
# 5. 后台批处理引擎 (核心黑科技)
# ==========================================
# 如果引擎处于打开状态，就开始切片扫描（每次扫30只，扫完自动刷新页面）
if st.session_state['is_scanning']:
    if current_count >= total_stocks:
        ...
        st.warning("⏳ **扫描引擎正在运行中，边扫边实时展示！你可以随时点击左侧【暂停扫描】**")
        ...
        st.rerun() # 🚀 神奇的代码：强制页面刷新，更新所有数据，并开启下一批次！

# ==========================================
# 6. 实时成果展示大屏
# ==========================================
st.markdown("### 🏆 实时捕获成果 (断点续传实时更新)")
